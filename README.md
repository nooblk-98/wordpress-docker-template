...
- `WORDPRESS_DB_WAIT_TIMEOUT` (default `60`) and `WORDPRESS_DB_WAIT_INTERVAL` (default `2`) for auto-install DB readiness wait loop (fails fast on non-retryable DB auth/database errors)
